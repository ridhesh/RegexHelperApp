# -*- coding: utf-8 -*-
"""
Created on Thu Nov 23 01:03:58 2017

@author: DAMI
"""

# @Author: AUTHOR NAME Das00130
# @Date:   2017-11-23
# @Filename: process.py
...
def fillnull(l, data): # This function fills all the null values in the particular columns and input are Config.json and List of data from dropnul function
    ...
    vals0 = list(evaldic.values())[0]    # Extracting the name of the columns and default values 
    vals1 = list(evaldic.values())[1]
    keys0 = list(evaldic.keys())[0]  # Convert keys to a list
    keys1 = list(evaldic.keys())[1]  # Convert keys to a list
    getindex0 = data[0].index(keys0)
    getindex1 = data[0].index(keys1)
    fullcolumndata=[data[0]]
    for record in data[1:]:   # Iterating through the data and checking the null columns and replacing null or nan with the default values 
...
def main(configFile):  # This is the main function which gets the input as the config.json file
    ...
    print("Config file read successfully:", config)
    myfile = config.get("inputfile")   # Getting the inputfile name from Config.json
    filename, ext = os.path.splitext(myfile)   #  Getting the extension of the file by splitting the filename
    print("Input file:", myfile, "Extension:", ext)
    ...
    print("Preprocessed data generated.")
    ...
    with open(config.get("outputfile"), mode="w") as out :  # Getting the name of the outputfile and write the data to it.
        ...
        for line in preprocesseddata:
            clean.writerow(line)
    print("Output file created:", config.get("outputfile"))
    return 
